<template>
  <div id="example-m" class="content-container">
    <div class="thd-map" id="changeMap">
      ...
      <div id="hisSearch">
        <!-- <Input v-model="hisSearchInfo" placeholder="输入历史地震" clearable style="width: 300px" /> -->
        <input
          type="text"
          v-model="hisSearchInfo"
          placeholder="输入历史地震"
          style="height:34px;width:170px;padding-left: 8px;"
        />
        <button
          @click="go()"
          style="width:32px;height:32px;background-color:#ffffff;border-width: 0px;font-size: 18px;"
        >
          <Icon type="ios-search" slot="suffix" />
        </button>
        <!-- <Input search placeholder="输入历史地震" on-click="go()"/> -->
  ...
  methods: {
    go() {
      ajax({
        ...
        .then((res) => {
          if(this.hisSearchInfo==="鲁甸地震6级" || this.hisSearchInfo === "鲁甸"){
            console.log(res.data);
            this.eqCenter = res.data;
            this.getNewThd(res.data.latitude, res.data.longitude);
          }else{
            this.$Message.warning('未找到此次地震');
          }
        })
        .catch((err) => {
          console.log(err);
    ...
    getNewThd(lat, lon) {
      ajax({
        method: "post",
        url:
          "http://" + pub.ip + "/subaoApi/public/api/obtain/getSearchThdData",
        params: {
          lat: lat,
          lon: lon,
        },
        ...
        .then((res) => {
          ...
          console.log(this.eqCenter, this.mapData);
          ajax({
            method: "post",
            url:
    ...
    getManualMarking() {
      ajax({
        method: "get",
        url: "http://localhost:81/subaoApi/public/api/getarcgis",
      })
        .then((res) => {
